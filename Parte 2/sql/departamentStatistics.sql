...
WITH salaries AS (
    SELECT
        ...
        COALESCE((
            SELECT SUM(v.valor)
            FROM vencimento v
            INNER JOIN emp_venc ev ON v.cod_venc = ev.cod_venc
...
ti AS (
    SELECT 
        de.nome AS nome_departamento,
		COUNT(*) as numero_empregados,
        ROUND(AVG(s.emp_salary), 2) AS media_salarial,
		MAX(s.emp_salary) AS maior_salario,
		MIN(s.emp_salary) AS menor_salario
    FROM salaries s
    INNER JOIN divisao di ON di.cod_divisao = s.lotacao_div
    INNER JOIN departamento de ON de.cod_dep = di.cod_dep
    WHERE de.nome = 'TI'
    GROUP BY de.nome
...
contabilidade AS (
    ...
    INNER JOIN divisao di ON di.cod_divisao = s.lotacao_div
    INNER JOIN departamento de ON de.cod_dep = di.cod_dep
    WHERE de.nome = 'Contabilidade'
    GROUP BY de.nome
    ORDER BY media_salarial DESC
)
SELECT * FROM ti
UNION ALL
SELECT * FROM engenharia
UNION ALL
